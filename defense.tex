...
\begin{frame}{What tools can combustion research use?}
    \hspace{6cm} \tikzmark{right}
    \begin{itemize}
        \item<1-> Experiments \tikzmark{2nd}
        \begin{itemize}[<2->]
    ...
    \begin{tikzpicture}[overlay, remember picture]
        \draw<4->[decoration={brace,amplitude=0.5em},decorate,ultra thick,red]
 ($(right)!(2nd.north)!($(right)-(0,1)$)$) --  ($(right)!(4th.south)!($(right)-(0,1)$)$);
        \path<4-> ($(right)!(2nd.north)!($(right)-(0,1)$)$) --  ($(right)!(4th.south)!($(right)-(0,1)$)$) node[midway,align=left,anchor=west,red,xshift=0.25cm] {These efforts\\are complementary!};
    \end{tikzpicture}
\end{frame}

\begin{frame}{What problem am I trying to solve?}
    \begin{itemize}
        \item n-butanol models in 2010 weren't very good
        \item sarathy made it better
        \item there's still work to do
    \end{itemize}
\end{frame}
    ...
    \begin{columns}[b]
        \column{0.65\textwidth}
            \begin{itemize}
                ...
                \item Models fail to predict certain engine relevant conditions, such as ignition delay dependence on [O$_2$]
            \end{itemize}
    \end{columns}
    }
    \only<4>{
