...
\begin{frame}{We use a lot of fuels to power the world}
    ...
    \only<2>{\begin{center}\includegraphics[height=0.8\textheight]{the-best-picture-ever}\end{center}}
    %Could drive to the moon and back over 180 million times in a Tesla Model S with the amount of energy we use annually}
    % \begin{center}
...
\begin{frame}{Better engines have higher efficiency and lower emissions}
    \begin{center}
        \includegraphics[height=0.85\textheight]{john-dec-engine}\\
        Reproduced from J.E. Dec, Proc. Combust. Inst. 32 (2009) 2727--2742
    \end{center}
\end{frame}

\begin{frame}{Better fuels reduce emissions and reduce dependence on fossil fuels}
    \begin{center}
    ...
    \begin{columns}
        ...
        \column{0.5\textwidth}
            ...
            \begin{tikzpicture}[scale=0.6]
                ...
                \node<2> at (0,0) {\includegraphics[width=0.9\textwidth]{computer}};
            \end{tikzpicture}
    \end{columns}
\end{frame}
...
\begin{frame}{What phenomena am I trying to understand?}
    \begin{center}
        ...
        \only<+>{
            ...
            Alternative fuels may undergo reactions that have not been described before
        }
    \end{center}
\end{frame}
...
\begin{frame}{Updated models better predict the combustion phenomena}
    \begin{center}
        \only<1>{
            \includegraphics[height=0.8\textheight]{nbuoh-2014/nbuoh-2014}\\
            Sarathy et al.\ --- With low temperature reactions\\
            Harper et al.\ --- Without low temperature reactions
        }
        \only<2>{
            \includegraphics[height=0.8\textheight]{mit-osswald-flame}\\
            Reproduced from Hansen et al.\ Combust. Flame 160 (2013) 2343--2351
        }
        \only<3>{
            \includegraphics[width=\textwidth]{llnl-osswald-flame}\\
            Reproduced from Sarathy et al.\ Combust. Flame 159 (2012) 2028--2055
        }
        \only<4>{
            \includegraphics[width=\textwidth]{llnl-mit-paths}\\
            Reproduced from Hansen et al.\ Combust. Flame 160 (2013) 2343--2351
        }
    \end{center}
\end{frame}
...
\begin{frame}{Why butanol?}
    \begin{columns}
        ...
        \column{0.5\textwidth}
            \centering
            \includegraphics[height=0.2\textheight]{1-butanol}\par
            \includegraphics[height=0.2\textheight]{2-butanol}\par
            \includegraphics[height=0.2\textheight]{t-butanol}\par
            \includegraphics[height=0.2\textheight]{iso-butanol}
    \end{columns}
\end{frame}
...
\begin{frame}{Sampling results}
    \begin{center}
        ...
        \draw<2-3> ($(mch)+(0,0.5)$) --     (0.3,7.3);
        \draw<3> ($(mchene-2)+(0,0.25)$) -- (0.75,3);
        \draw<3> ($(mchene-3)+(0,0.25)$) -- (0.85,3);
        \draw<3> ($(menech)+(0,0.25)$) --   (1,1.9);
        \draw<3> ($(mchene-1)+(0,0.5)$) -- (2.4,2);
        \draw<4> ($(2-propenal)$) -- (-3.9,3);
        \draw<4> ($(mac)$) -- (-3.4,2);
        \draw<4> ($(mvk)$) -- (-3.2,2);
        \end{tikzpicture}
        }
...
\begin{frame}{Future work}
    \only<1>{
        \begin{itemize}
            ...
            \item Resolve discrepancies at early sampling times
